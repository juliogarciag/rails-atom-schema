...
function handleEditorChange(editor, dispatch, state) {
  ...
  startModelLoadingProcess(modelPath, dispatch);
}

async function startModelLoadingProcess(modelPath, dispatch) {
  dispatch(startModelLoading());

  try {
    const { modelName } = await getModelInfo(modelPath);
...
function getModelInfo(modelPath) {
  return callCommand("ruby", [getRubyScript("model_info"), modelPath]);
}

function getModelSchemaInfo(modelPath, modelName) {
  const railsRoot = findRailsRoot(modelPath);
  if (!railsRoot) {
    throw new Error(NO_RAILS_PROJECT_MESSAGE);
  }
